...
class SST(nn.Module):
    def __init__(
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches, embed_dim))  # Learnable embedding
        # self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, embed_dim))
        # self.cls_token = nn.Parameter(torch.randn(1, 1, embed_dim))
        self.dropout = nn.Dropout(emb_dropout)

        self.encoder_layer = nn.TransformerEncoderLayer(d_model=embed_dim, nhead=num_heads, dim_feedforward=embed_dim * num_heads, dropout=dropout, activation='gelu')
